...
- e7bd767: Extraction now copies each field from the document as written — wording, spelling, casing, and punctuation — instead of rewriting dates, addresses, names, or phrasing into a canonical form. Ask for a specific format in the field's schema type or description when you need one; otherwise apply formatting in your own downstream step. Keeping the source wording means you still have the document's actual text, and grounded review can check the value against that text instead of a silently rewritten string.
